    ...
    if(clientIsConnected(&_client) && _client.status == WSC_CONNECTED) {
        if(!headerToPayload) {
            ...
            if(ret) {
                ...
                ret            = WebSocketsClient::write(&_client, buf, 2); // gui den server kieu data, tieu de gui
            }
            if(ret && payload && length > 0) {
                ret = WebSocketsClient::write(&_client, payload, length);// gui noi dung toi server 
            }
            return ret;
        } else {
            // TODO implement
